...
export default async function CategoryPage({
  ...
  // For turboladdare: real products first, then generated to pad out the catalog
  const realProducts: DisplayProduct[] = slug === "turboladdare"
  ...
  // Merge: real products at the top, generated fill the rest (avoid duplicate SKUs)
  const realSkus = new Set(realProducts.map((p) => p.sku));
  const filtered = generated.filter((p) => !realSkus.has(p.sku));
  const ALL_PRODUCTS: DisplayProduct[] = [...realProducts, ...filtered];
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-6">
              ...
              <ul className="space-y-2">
                {filterBrands.map((brand) => (
                  <li key={brand} className="flex items-center gap-2">
                    ...
                    <label htmlFor={brand} className="text-sm text-gray-700 cursor-pointer hover:text-red-600 transition">
                      {brand}
                    </label>
                  </li>
                ))}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm text-gray-500">
                ...
                {realProducts.length > 0 && (
                  <span className="ml-2 text-green-600 font-medium">· {realProducts.length} i lager</span>
                )}
              </p>
            ...
            <div className="flex items-center justify-center gap-1 mt-10">
              {currentPage > 1 && (
                <Link
                  href={`/kategori/${slug}?sida=${currentPage - 1}`}
